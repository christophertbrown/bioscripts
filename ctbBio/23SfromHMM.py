...
"""
script for 1) finding 23S rRNA gene sequences from contigs based on ssu-cmsearch or cmsearch tabular output and 2)
identifying 23S rRNA gene insertions
...
def check_overlap(current, hit, overlap = 200):
    ...
    indicating that this hit is for another 23S rRNA gene
    """
    for prev in current:
        p_coords = prev[2:4]
        coords = hit[2:4]
...
def find_coordinates(hmms, bit_thresh):
    """
    find 23S rRNA gene sequence coordinates
    """
    ...
    for seq, info in list(seq2hmm.items()):
        ...
        for group_num, group in enumerate(hit_groups(info[1])): 
            # group is a group of hits to a single 23S gene
...
def find_23S(fastas, hmms, bit_thresh = float(20), length_thresh = 500, masking = True, buffer = 0):
    ...
    3) identify regions that match to 23S (for best model)
    4) mask internal regions that do not align to model
    5) length threshold applies to aligned regions of 23S sequence
    5) export 23S sequnece based on complete gene (including masked insertions)
    """
...
def run_cmsearch(fastas, threads, cm):
    """
    run cmsearch: comapre 23S sequences to ssu-align's CM
    """
    out = []
    for fasta in fastas:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='# find 23S rRNA genes using cmsearch')
    parser.add_argument(\
    ...
    parser.add_argument(\
            ...
            help = 'minimum length 23S rRNA gene sequence to report, default = 500')
    ...
    if args['c'] is False:
        if args['d'] is False and 'lsucmdb' not in os.environ:
            print('specify: -d <cmsearch database>', file=sys.stderr)
            exit()
        if args['d'] is False:
            cm = os.environ['lsucmdb']
            print('# 23S rRNA CM: %s' % (cm), file=sys.stderr)
        else:
            cm = args['d']
    ...
    for seq in find_23S(fastas, hmms, bit_thresh, length_thresh, masking, buffer):
        print('\n'.join(seq))
